...
function util.GaussianKernel(size, sigma_horz, sigma_vert)
    sigma = sigma_horz
    kernel_image = image.gaussian(size, sigma/size, 1, true, size, size, sigma_horz, sigma_vert):double()
    return kernel_image
end

-- Src must be HW ByteTensor, output will be ByteTensor
function util.GaussianSmoothing(src, kernel_image)
    dst = image.convolve(src:double(), kernel_image, 'same'):float()
    return dst
end
