...
class ModelJoint(nn.Module):
    ...
    def __init__(self):
        ...
        self.base_conv = nn.Sequential(
                          ixvr(nn.Conv2d(3, 20, 7, stride=1, padding=3)), # 3x101x101 -> 20x101x101
                          nn.ReLU(inplace=True), 
                          nn.MaxPool2d(2, stride=2), # 20x50x50
                          ixvr(nn.Conv2d(20, 40, 5, stride=1, padding=2)), # 40x50x50
                          nn.ReLU(inplace=True),
                          nn.MaxPool2d(2, stride=2), # 40x25x25
                          ixvr(nn.Conv2d(40, 80, 4, stride=1, padding=1)), # 80x24x24
                          nn.ReLU(inplace=True),
                          nn.MaxPool2d(2, stride=2), # 80x12x12
                          ixvr(nn.Conv2d(80, 160, 4, stride=2, padding=1)), # 160x6x6
                          nn.ReLU(inplace=True),
                          nn.MaxPool2d(2, stride=2)) # 160x3x3

        self.base_fc = nn.Sequential(
                          ixvr(nn.Linear(1440, 500)), 
                          nn.ReLU(inplace=True),
                          nn.Dropout(p=0.5),
                          ixvr(nn.Linear(500, 500)),
                          nn.ReLU(inplace=True),
                          nn.Dropout(p=0.5))
        
        self.pose_regressor = ixvr(nn.Linear(1000, 6))
        self.matcher = ixvr(nn.Linear(1000, 1))
